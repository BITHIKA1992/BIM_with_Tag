...
### Random Node + Random Tag as SeedNode and SeedTag

print("*****************************************************************************************************")
print("***********      BaseLine-I TBIM- RN+RT -- SeedSet and Seed Tag Selection ***************************")
...
random.shuffle(sorted_1000tags)
...
for b in range(len(budget)):
    ...
    S_prime = []
    total_user_cost = 0.0
    while total_user_cost <= B_u and B_u - total_user_cost >= min(cost_per_user.values()) :
        user = random.sample( list(DG_prime.nodes()) , 1 )[0]
    ...
    T_prime = []
    total_tag_cost = 0.0
    while total_tag_cost <= B_t and B_t - total_tag_cost >= min(cost_per_mtag.values()) :
        tag  = random.sample( sorted_1000tags , 1 )[0]
...
with open('result_1_RN_RT.pickle', 'wb') as handle:
